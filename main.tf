...
resource "azurerm_sql_firewall_rule" "sqldw_fw_rules" {
  ...
  name                = var.whitelisted_networks[count.index]["name"]
  resource_group_name = var.resource_group_name
  server_name         = azurerm_sql_server.sqldw_server.name
  start_ip_address    = var.whitelisted_networks[count.index]["start_ip"]
  end_ip_address      = var.whitelisted_networks[count.index]["end_ip"]
}
